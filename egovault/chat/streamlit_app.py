...
from egovault.agent.commands import HELP_MD as _HELP_MD  # noqa: E402 — after page config
...
def _init_scheduler():
    ...
    from egovault.chat.session import _register_auto_schedules
...
def _init_background_tasks():
    ...
    from egovault.chat.session import _start_background_tasks
...
def _init_profile_thread() -> dict:
    ...
    def _run() -> None:
        from egovault.chat.rag import extract_owner_profile
...
if prompt:
    ...
    if lower == "/help":
        ...
        st.session_state.messages += [
            ...
            {"role": "assistant", "content": _HELP_MD},
        ]
        st.stop()

    if lower in ("/clear", "/restart"):
        st.session_state.messages = []
        st.session_state.conv_history = []
        st.session_state.sources = []
        st.rerun()
    ...
    if lower == "/sources":
        reply = (
            "\n".join(f"- {s}" for s in st.session_state.sources)
            if st.session_state.sources else "_No sources from previous answer._"
        )
        with st.chat_message("user"):
            st.markdown(prompt)
        with st.chat_message("assistant"):
    ...
    if lower == "/profile":
        profile = st.session_state.owner_profile or "_No profile extracted yet — extraction runs automatically in the background._"
        with st.chat_message("user"):
            st.markdown(prompt)
        with st.chat_message("assistant"):
            st.markdown(profile)
    ...
    if lower == "/status":
        lines: list[str] = []
        # Vault stats — open a fresh connection on this thread to avoid SQLite
        # "created in thread X, used in thread Y" errors from the cached store.
        try:
            from egovault.core.store import VaultStore as _VS
            _stat_store = _VS(settings.vault_db)
            _stat_store.init_db()
            try:
                result = _stat_store.count_records()
            finally:
                _stat_store.close()
            lines.append(f"**Vault records:** {result['total']}")
        ...
        # Ollama model stats
        try:
            from egovault.chat.session import _ollama_ps, _fmt_bytes
            models = _ollama_ps(settings.llm.base_url)
            if models:
                lines.append("")
                ...
                lines.append("_No models currently loaded in Ollama._")
        except Exception:
            pass
        # Top-N + background tasks
        lines.append(f"\n**Top-N chunks:** {st.session_state.top_n} (auto)")
        active_bg = [t for t in _bg_threads if t.is_alive()]
        if active_bg:
            lines.append("**Background tasks running:** " + ", ".join(t.name.replace("bg-","") for t in active_bg))
        sched_tasks = scheduler.list_tasks()
        if sched_tasks:
            lines.append(f"**Scheduled tasks:** {len(sched_tasks)}")
        reply = "\n".join(lines)
        with st.chat_message("user"):
            st.markdown(prompt)
        with st.chat_message("assistant"):
    ...
    # If the user's message contains a time expression ("in 5min", "every hour",
    # etc.) and isn't already a slash command, convert it to a /schedule command
    # before falling through to the /schedule handler below.
    if not lower.startswith("/"):
        from egovault.chat.session import _resolve_schedule_intent
        ...
        if not arg or arg.lower() in ("--list", "-l", "list"):
            ...
            try:
                ...
                if arg.lower().startswith("/gmail-sync"):
                    ...
                    from egovault.chat.session import _SCHEDULE_TIME_RE as _stre
                    import re as _re_sc
    ...
    # --- Normal message: LLM pipeline ---

    st.session_state.messages.append({"role": "user", "content": prompt})
    with st.chat_message("user"):
        st.markdown(prompt)

    # If files were uploaded since the last query, inject a context hint so the
    # agent knows to search them by name rather than relying on keyword guessing.
    _upload_hint = ""
    ...
    sources_snapshot: list = list(st.session_state.sources)
    owner_profile: str = st.session_state.owner_profile
    ...
    from egovault.chat.rag import build_prompt, source_attribution
    ...
    with st.chat_message("assistant"):
        ...
        if llm_cfg.provider in ("ollama", "llama_cpp"):
            ...
            hyde_kwargs = {**llm_kwargs, "timeout": min(15, llm_cfg.timeout_seconds)}
            session_ctx: dict = {
                "settings": settings,
                "last_sources": sources_snapshot,
                "owner_profile": owner_profile,
                "owner_profile_ref": {},
                "call_llm_fn": _call_llm,
                "hyde_llm_kwargs": hyde_kwargs,
                "scheduler": scheduler,
            ...
            def _run() -> None:
                ...
                try:
                    ans, _data, chunks = _call_llm_agent(
                        initial_messages, thread_store, top_n, llm_kwargs, None,
                        session_ctx=session_ctx, progress_cb=_progress_cb,
                    )
                    result_box["answer"] = ans
                    result_box["chunks"] = chunks
                    result_box["profile_dirty"] = session_ctx["owner_profile_ref"].get("dirty", False)
                    result_box["saved_attachments"] = session_ctx.get("saved_attachments", [])
                except Exception as exc:
            ...
            with st.status("Thinking...", expanded=True) as status:
                emitted = 0
                _sync_placeholder = None  # reusable placeholder for gmail_sync progress
                while not done_event.wait(timeout=0.4):
                    for line in progress_lines[emitted:]:
                        if line.startswith("⚙ Syncing emails"):
                            # Reuse a single placeholder so progress updates replace
                            # each other rather than stacking into hundreds of lines.
                            if _sync_placeholder is None:
                                _sync_placeholder = st.empty()
                            _sync_placeholder.write(f"- {line}")
                        else:
                            _sync_placeholder = None  # next sync starts fresh
                            st.write(f"- {line}")
                        emitted += 1
                for line in progress_lines[emitted:]:
                    if line.startswith("⚙ Syncing emails"):
                ...
                if "error" in result_box:
                    ...
                    answer = f"**Error:** {result_box['error']}"
                else:
                    status.update(label="Done", state="complete")
                    answer = result_box.get("answer", "")
                    collected_chunks = result_box.get("chunks", [])
                    _saved_attachments = result_box.get("saved_attachments", [])

            if result_box.get("profile_dirty"):
                # A scan happened — vault has new data, re-extract profile in background.
            ...
            from egovault.chat.rag import (
                assemble_context, plan_search_queries, retrieve, vault_summary_context,
            )
            from egovault.core.store import VaultStore as _VS

            plan_kwargs = {**llm_kwargs, "timeout": min(15, llm_cfg.timeout_seconds)}

            with st.spinner("Planning search..."):
                try:
                    planned = plan_search_queries(prompt, conv_history, _call_llm, plan_kwargs)
                except Exception:
                    planned = None

            simple_store = _VS(settings.vault_db)
            simple_store.init_db()
            try:
                with st.spinner("Searching vault..."):
                    collected_chunks = retrieve(
                        simple_store, prompt, top_n=top_n,
                        planned_query=planned,
                        reranker_cfg=settings.reranker,
                        embed_cfg=settings.embeddings,
                        llm_base_url=llm_cfg.base_url,
                        call_llm_fn=_call_llm,
                        llm_kwargs=plan_kwargs,
                        crag_cfg=settings.crag,
                        sw_cfg=settings.sentence_window,
                    )
                context = assemble_context(collected_chunks) or vault_summary_context(simple_store) or ""
                if _upload_hint:
                    context = f"{_upload_hint}\n\n{context}" if context else _upload_hint
            finally:
                simple_store.close()
        ...
        sources = source_attribution(collected_chunks) if collected_chunks else []
        ...
        # Render any images captured by fetch_attachment / inspect_windows.
    ...
    # Store a compact history entry for screen-tool answers so the LLM can't
    # echo the window list verbatim on the next turn — it must re-call the tool.
